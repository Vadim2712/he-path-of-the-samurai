...
impl SpaceService {
    ...
    async fn fetch_and_cache(&self, url: &str, source_key: &str, params: &[(&str, &str)]) -> anyhow::Result<()> {
        info!("Fetching data for {}", source_key);
        
        let mut query_params = params.to_vec();
        if !self.state.nasa_key.is_empty() {
            query_params.push(("api_key", &self.state.nasa_key));
        }
        ...
        if !response.status().is_success() {
            ...
            error!("API request for {} failed with status {}: {}", source_key, status, body);
            anyhow::bail!("API request for {} failed with status {}", source_key, status);
        }
        ...
        let data: serde_json::Value = match serde_json::from_slice(&response_bytes) {
            ...
            Err(e) => {
                ...
                error!("Failed to parse JSON for {}. Body: {}", source_key, body_text);
                return Err(e.into());
            }
        };
    ...
    pub async fn fetch_apod(&self) -> anyhow::Result<()> {
        self.fetch_and_cache(self.state.apod_url.as_str(), "apod", &[("thumbs", "true")]).await
    }
    ...
    pub async fn fetch_neo(&self) -> anyhow::Result<()> {
        ...
        self.fetch_and_cache(self.state.neo_url.as_str(), "neo", &[
    ...
    pub async fn fetch_donki(&self) -> anyhow::Result<()> {
        ...
        let params = [("startDate", start_date.as_str()), ("endDate", end_date.as_str())];

        self.fetch_and_cache(self.state.donki_flr_url.as_str(), "flr", &params).await?;
        self.fetch_and_cache(self.state.donki_cme_url.as_str(), "cme", &params).await?;

        Ok(())
    }

    pub async fn fetch_spacex_next(&self) -> anyhow::Result<()> {
        self.fetch_and_cache(self.state.spacex_next_url.as_str(), "spacex", &[]).await
    }
}
